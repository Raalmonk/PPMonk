...
class MonkEnv(gym.Env):
    ...
    def step(self, action_idx):
        ...
        # [Pure Reward] No shaping, no punishment.
        reward = step_damage 
...
def run():
    print(">>> 初始化训练环境 (Brute Force: 2M Steps, Large Net)...")
    env = MonkEnv()
    env.reset()
    ...
    # [核心升级]
    # 1. Gamma = 1.0: 绝对公平，20秒后的伤害和现在一样值钱
    # 2. Net Arch = [256, 256]: 更大的脑容量来记忆长序列
    # 3. Ent Coef = 0.05: 更强的探索欲，强迫它尝试"不按技能"
    model = MaskablePPO(
        "MlpPolicy", 
        env, 
        verbose=1, 
    ...
    print(">>> 开始训练 (Steps: 2,000,000)... 这可能需要一点时间")
