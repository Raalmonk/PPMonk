...
* **Flurry Charges**: Generated by AA (2H 100%, DW ~74%).
    * **One Versus Many**: AA Crit generates 2 stacks.
    * **Stand Ready**: Cast Zenith -> Gain 10 Stacks. Next AA triggers Flurry @ 70% effectiveness.
* **Flurry Strikes**:
    * Triggered by FOF (consumes all) or Stand Ready AA.
    * **Physical** Damage: `0.6 AP` per stack (Subject to Armor).
    * **Shado Over the Battlefield**: Adds `0.52 AP` **Nature** AOE (Soft Cap **8**).
    * **High Impact**: Adds `1.0 AP` **Physical** Explosion (Soft Cap 8).
* **Passives**:
    * **Martial Precision**: Armor Pen `12%`.
    * **Against All Odds**: Agility `+4%`.
    * **Vigilant Watch**: BOK Crit Dmg `+30%`.
    * **Wisdom of the Wall**: Zenith active -> RSK/SCK trigger 3 Flurry Strikes.
...
* **Invoke Xuen**:
    * **Tiger Lightning**: **Nature** damage (0.257 AP).
    * **Empowered Lightning**: **Nature** damage (8% of recent dmg).
* **Celestial Conduit**:
    * Channel 4s. **Nature** AOE (`5 * 2.75 AP` per sec).
    * **Unity Within**: Final burst of damage.
* **Heart of Jade Serpent**:
    * Trigger: SOTWL/WDP.
    * CDR Rate `+75%` for 8s. FOF Channel time `-50%`.